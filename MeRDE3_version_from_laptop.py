...
def ls(path, key='.*', direc=False, rec=True, n=None, m=None, sep=None):
	fio = []
	if(path[-1] != '/'):
		path += '/'
	if not(os.path.isdir(path)):
		exit('invalid path given for lsfio search: ' +path)
	regex = re.compile(key)
	if(direc):																		#search for folders instead of files
		if(rec):																	#recursive search case
			for r,d,f in os.walk(path):
				for x in d:
					if(regex.search(x)):
						fio.append(r +'/' +x +'/')
		else:																		#non-recursive search case
			for x in os.listdir(path):
				if(regex.search(x) and os.path.isdir(path +x +'/')):
					fio.append(path +x +'/')
		if(n and m and sep):
			try:
				fio.sort(key=lambda x: (int(x.split('/')[-1].split(sep)[n]),int(x.split('/')[-1].split(sep)[m])))
			except ValueError:
				try:
					fio.sort(key=lambda x: (x.split('/')[-1].split(sep)[n],int(x.split('/')[-1].split(sep)[m])))
				except ValueError:
					fio.sort(key=lambda x: (x.split('/')[-1].split(sep)[n],x.split('/')[-1].split(sep)[m]))
		else:
			fio.sort()
		for i in range(len(fio)):
			fio[i] = fio[i].replace('//','/')
		return(fio)
	else:																			#search for files instead of folders
		if(rec):																	#recursive search case
			for r,d,f in os.walk(path):
				for x in f:
					if(regex.search(x)):
			...
			for x in os.listdir(path):
				if(regex.search(x) and not os.path.isdir(path +x +'/')):
					fio.append(path +x)
		if(n and m and sep):
			try:
				fio.sort(key=lambda x: (int(x.split('/')[-1].split(sep)[n]),int(x.split('/')[-1].split(sep)[m])))
...
#####ANALYZE MODE
	...
	parser.add_argument('-o', '-output_dir', help='Directory where output will be saved. [Default: current working directory]', metavar='<Path>', default=os.getcwd() +'/')
	parser.add_argument('-zzz', help='', metavar='<Path>')
	parser.add_argument('-q', '-quiet', help='Do not write status information into console.', action='store_true')
	args = parser.parse_args()
	...
	def boxplot(geneID, outlierFilteredNormCMatrix, pV, path):
		boxplot.count += 1
		...
		plt.savefig(f'{path}/boxplots/{boxplot.count}_{geneID}.png')
		plt.close(fig)
	...
	def boxplot3(geneID, someCountMatrix, pV, path):
...
############## Outlier filtering
	...
	if(args.e != 'ignore'):
		...
		outlierFilteredNormCMatrix = filteredLowNormCMatrix
##############
##############
...
############## Write HTML Output
	...
	if(args.no_html == False):
		...
		# output_fileoutput_file(f'{args.o}/html/ma_plot.html')

		# resData = get_results_data()
		# save(draw_mfc_plot(resData))
		if not(os.path.exists(f'{args.zzz}/boxplots/')):
			os.makedirs(f'{args.zzz}/boxplots/')

		boxplot.count = 0
		for gen in result:
			boxplot(gen[0], outlierFilteredNormCMatrix, gen[2], f'{args.zzz}')
		#still to do
	##############
	##############
	##############
